...
video_path = 'calibrate_v3/videos/cam2/calib2.mov'
...
out = cv2.VideoWriter('cam2calib2.mp4', fourcc, 30.0, (frame_width, frame_height))
...
while cap.isOpened():
    ...
    if ret:
        corners2 = cv2.cornerSubPix(gray, corners, (16, 16), (-1, -1), criteria)
        objpoints.append(objp)
        imgpoints.append(corners2)

        # Draw corners and write the frame
        cv2.drawChessboardCorners(frame, (9, 7), corners2, ret)
        out.write(frame)
...
if len(objpoints) > 0 and len(imgpoints) > 0 and frame_shape is not None:
    ...
    df_mtx.to_csv('cam2_camera_matrix.csv', index=False)
    df_dist.to_csv('cam2_distortion_coefficients.csv', index=False)

    # Save to XML with descriptors
    fs = cv2.FileStorage('cam2_calibration_parameters.xml', cv2.FileStorage_WRITE)
