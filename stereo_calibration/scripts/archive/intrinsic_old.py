...
def create_directories(test_dir):
    ...
    # Temp directories - create BOTH variants to ensure compatibility
    temp_dir = os.path.join(test_dir, "temp")
    os.makedirs(os.path.join(temp_dir, "left"), exist_ok=True)
    os.makedirs(os.path.join(temp_dir, "right"), exist_ok=True)
    os.makedirs(os.path.join(temp_dir, "left_camera"), exist_ok=True)
    os.makedirs(os.path.join(temp_dir, "right_camera"), exist_ok=True)
...
def extract_frames(video_path, output_dir, camera_id, frame_interval=10, max_frames=50):
    """
    ...
    while cap.isOpened() and saved_count < max_frames:
        ...
        if frame_count % frame_interval == 0:
            # Save frame to BOTH directory variants for compatibility
            frame_path1 = os.path.join(output_dir, camera_id, f"{camera_id}_frame_{saved_count:04d}.png")
            frame_path2 = os.path.join(output_dir, f"{camera_id}_camera", f"{camera_id}_frame_{saved_count:04d}.png")
            
            cv2.imwrite(frame_path1, frame)
            # Create a copy in the alternate location
            os.makedirs(os.path.dirname(frame_path2), exist_ok=True)
            shutil.copy2(frame_path1, frame_path2)
...
def save_calibration_results(camera_matrix, dist_coeffs, error, base_dir, camera_id):
    ...
    results_dir = os.path.join(base_dir, "results", "intrinsic_params")
...
def main():
    ...
    for camera_id in ['left', 'right']:
        ...
        camera_dir = f"{camera_id}_camera"
        video_path = os.path.join(test_dir, camera_dir, "intrinsic.mp4")
